...
The graph objects returned by loadgraph and inputgraph are by default constructed using the <graph> class in the
module basicgraphs.py, but by using an optional argument you can use your own graph class (provided that it supports
the same methods/interface).
...
from experiment.structures import basicgraphs
...
def readgraph(graphclass, readline):
    ...
    for edge in edgelist:
        ...
        e = G.add_edge(G[edge[0]], G[edge[1]])
